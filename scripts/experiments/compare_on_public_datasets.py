        ...
        for masks in [x for x in masks_dir.glob('*')][:1]:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                for mask in loaded_masks:
                    ...
                    def _worker(mask_worker):
                        fr_text_file_w, en_text_file_w = mask_worker['fr'], mask_worker['en']
                        equal = mask_worker['equal']
                        fr_text, en_text = contents_by_filenames[fr_text_file_w], contents_by_filenames[en_text_file_w]
                        y = int(labse_detector.count_similiarity_whole_text(fr_text, en_text) > labse_detector.sim_threshold)
                        return y + equal * 2
                    futures.append(executor.submit(_worker, mask))
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    print_idx += 1
                    if not print_idx % 500:
                        print(f'Intermediate result tn_fp_fn_tp: {tn_fp_fn_tp}')
